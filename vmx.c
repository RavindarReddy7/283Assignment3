...
{
	extern atomic_long_t total_cpu_cycles_per_exit[69];
	struct vcpu_vmx *vmx = to_vmx(vcpu);
	...
	if(exit_reason.basic <69){
	 atomic64_inc(&exit_counter[exit_reason.basic]);
	 atomic64_add((rdtsc()-start_time), &total_cpu_cycles_per_exit[exit_reason.basic]);
	 }
